...
export type SchemaGroupMenuProps = {
  group: SchemaGroup;
...
function SchemaGroupCardComponent({
  ...
  return (
    <section
      /* The canvas resolves a right-click to a group through this: the body is
         `pointer-events: none`, so only the head and the resize handle can be
         found by walking the DOM -- the rest is found geometrically. */
...
/**
 * The group's context-menu rows, rendered by the canvas rather than by the card:
 * the group's body is `pointer-events: none` so the canvas can be panned and
 * marquee-selected through it, which means the right-click never lands on this
 * element at all. The canvas owns the one trigger and decides which menu it is.
 */
export function SchemaGroupMenu({
  group,
  pendingPrefix,
  readOnly,
  onPatch,
