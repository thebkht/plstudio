...
export default function TemplateGallery({ workspace }: { workspace?: string }) {
  ...
  return <main className="templates-page"><header className="templates-header"><a href={homeHref} className="brand-link"><BrandMark /></a><div className="templates-title"><HugeiconsIcon icon={Layout01Icon} size={18} /> Templates</div><a className="templates-back" href={homeHref}>Back to projects</a></header><section className="templates-intro"><p className="section-kicker">Start with a strong foundation</p><h1>Database schema templates</h1><p>A collection of practical diagrams to give your next project a quick start or a little inspiration.</p></section><div className="template-tabs"><button className="active" type="button">Default templates</button><button type="button" disabled>Your templates</button></div><section className="template-grid">{templates.map((template) => <article className="template-card" key={template.name}><Preview accent={template.accent} /><div className="template-card-copy"><div className="template-meta"><span>{template.kind}</span><HugeiconsIcon icon={DatabaseIcon} size={14} /></div><h2>{template.name}</h2><p>{template.description}</p><button className="template-action" type="button" disabled={busy !== null} onClick={() => void useTemplate(template.name)}>{busy === template.name ? "Creating…" : "Use template"}<HugeiconsIcon icon={GitForkIcon} size={15} /></button></div></article>)}</section><footer className="templates-footer">DrawSQL · Visual Oracle schema design</footer></main>;
}
