...
export async function POST(request: Request) {
  try { const input = (await request.json()) as { workspace?: string; name?: string }; if (!input.name?.trim()) return Response.json({ error: "name is required" }, { status: 400 }); const workspaceAccess = input.workspace ? await requireWorkspace(input.workspace) : null; const session = workspaceAccess?.session ?? await requireSession(); const starter = makeEmptySchema(input.name.trim(), crypto.randomUUID()); const row = { id: starter.id, name: starter.name, organizationId: workspaceAccess?.organization.id ?? null, createdBy: session.user.id, schemaJson: starter, revision: 1, schemaFormatVersion: SCHEMA_FORMAT_VERSION }; await getDb().insert(projects).values(row); return Response.json(starter, { status: 201 }); }
