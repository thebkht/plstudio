...
export default async function PersonalProjectPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <main className="app-shell"><Designer initialSchema={project.schemaJson as Schema} projectId={project.id} user={{ id: session.user.id, name: session.user.name, image: session.user.image }} /></main>;
}
